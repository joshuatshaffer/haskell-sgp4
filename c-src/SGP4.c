...
*                               sgp4unit.cpp
*
*    this file contains the SGP4Funcs_sgp4 procedures for analytical propagation
...
/* ----------- local functions - only ever used internally by SGP4Funcs_sgp4
 * ---------- */
static void dpper (double e3, double ee2, double peo, double pgho, double pho,
                   ...
                   double xl3, double xl4, double zmol, double zmos,
                   double inclo, char init, double *ep, double *inclp,
                   double *nodep, double *argpp, double *mp, char opsmode);

static void
dscom (double epoch, double ep, double argpp, double tc, double inclp,
       double nodep, double np, double *snodm, double *cnodm, double *sinim,
       double *cosim, double *sinomm, double *cosomm, double *day, double *e3,
       double *ee2, double *em, double *emsq, double *gam, double *peo,
       double *pgho, double *pho, double *pinco, double *plo, double *rtemsq,
       double *se2, double *se3, double *sgh2, double *sgh3, double *sgh4,
       double *sh2, double *sh3, double *si2, double *si3, double *sl2,
       double *sl3, double *sl4, double *s1, double *s2, double *s3, double *s4,
       double *s5, double *s6, double *s7, double *ss1, double *ss2,
       double *ss3, double *ss4, double *ss5, double *ss6, double *ss7,
       double *sz1, double *sz2, double *sz3, double *sz11, double *sz12,
       double *sz13, double *sz21, double *sz22, double *sz23, double *sz31,
       double *sz32, double *sz33, double *xgh2, double *xgh3, double *xgh4,
       double *xh2, double *xh3, double *xi2, double *xi3, double *xl2,
       double *xl3, double *xl4, double *nm, double *z1, double *z2, double *z3,
...
static void dsinit (
    ...
    double xke, double cosim, double emsq, double argpo, double s1, double s2,
    double s3, double s4, double s5, double sinim, double ss1, double ss2,
    double ss3, double ss4, double ss5, double sz1, double sz3, double sz11,
    double sz13, double sz21, double sz23, double sz31, double sz33, double t,
    double tc, double gsto, double mo, double mdot, double no, double nodeo,
    double nodedot, double xpidot, double z1, double z3, double z11, double z13,
    double z21, double z23, double z31, double z33, double ecco, double eccsq,
    double *em, double *argpm, double *inclm, double *mm, double *nm,
    double *nodem, int *irez, double *atime, double *d2201, double *d2211,
    double *d3210, double *d3222, double *d4410, double *d4422, double *d5220,
    double *d5232, double *d5421, double *d5433, double *dedt, double *didt,
    double *dmdt, double *dndt, double *dnodt, double *domdt, double *del1,
    double *del2, double *del3, double *xfact, double *xlamo, double *xli,
    double *xni);

static void dspace (int irez, double d2201, double d2211, double d3210,
                    ...
                    double xfact, double xlamo, double no, double *atime,
                    double *em, double *argpm, double *inclm, double *xli,
...
static void dpper (double e3, double ee2, double peo, double pgho, double pho,
    ...
    double alfdp, betdp, cosip, cosop, dalf, dbet, dls, f2, f3, pe, pgh, ph,
        pinc, pl, sel, ses, sghl, sghs, shll, shs, sil, sinip, sinop, sinzf,
        sis, sll, sls, xls, xnoh, zf, zm, zel, zes, znl, zns;
...
*                           procedure dsinit
*
*  this procedure provides deep space contributions to mean motion SGP4Funcs_dot
due
...
*    mo          - mean anomaly
*    mdot        - mean anomaly SGP4Funcs_dot (rate)
*    no          - mean motion
*    nodeo       - right ascension of ascending node
*    nodedot     - right ascension of ascending node SGP4Funcs_dot (rate)
*    xpidot      -
...
*
*  coupling      :
*    SGP4Funcs_getgravconst- no longer used
*
*  references    :
*    hoots, roehrich, norad spacetrack report #3 1980
*    hoots, norad spacetrack report #6 1986
...
static void dsinit (
    // sgp4fix just send in xke as a constant and eliminate
    // SGP4Funcs_getgravconst call
    // gravconsttype whichconst,
    double xke, double cosim, double emsq, double argpo, double s1, double s2,
    double s3, double s4, double s5, double sinim, double ss1, double ss2,
    double ss3, double ss4, double ss5, double sz1, double sz3, double sz11,
    ...
    double ainv2, aonv = 0.0, cosisq, eoc, f220, f221, f311, f321, f322, f330,
                  ...
                  g310, g322, g410, g422, g520, g521, g532, g533, ses, sgs,
                  sghl, sghs, shs, shll, sis, sini2, sls, temp, temp1, theta,
                  xno2, q22, q31, q33, root22, root44, root54, rptim, root32,
    ...
    // SGP4Funcs_getgravconst( whichconst, tumin, mu, radiusearthkm, xke, j2,
    // j3, j4,
    // j3oj2 );
    ...
    *mm = *mm + *dmdt * t;
    ...
    if (*irez != 0) {
        ...
        /* ------------ for SGP4Funcs_sgp4, initialize the integrator ----------
         */
        *xli = *xlamo;
        *xni = no;
        *atime = 0.0;
...
*    irez        - flag for resonance           0-none, 1-one day, 2-half day
*    argpo       - argument of perigee
*    argpdot     - argument of perigee SGP4Funcs_dot (rate)
*    t           - time
*    tc          -
*    gsto        - gst
*    xfact       -
...
static void dspace (int irez, double d2201, double d2211, double d3210,
    ...
    *mm = *mm + dmdt * t;
    ...
    if (irez != 0) {
        ...
        while (iretn == 381) {
            /* ------------------- SGP4Funcs_dot terms calculated -------------
             */
...
*  coupling      :
*    SGP4Funcs_getgravconst- no longer used
*    SGP4Funcs_gstime      - find greenwich sidereal time from the julian date
*
*  references    :
*    hoots, roehrich, norad spacetrack report #3 1980
*    hoots, norad spacetrack report #6 1986
...
static void initl (
    ...
    *method = 'n';
    ...
    //    }
    //    else
    *gsto = SGP4Funcs_gstime (epoch + 2433281.5);
...
/*-----------------------------------------------------------------------------
*
*                             procedure SGP4Funcs_sgp4init
*
*  this procedure initializes variables for SGP4Funcs_sgp4.
*
*  author        : david vallado                  719-573-2600   28 jun 2005
*
*  inputs        :
...
*    bstar       - SGP4Funcs_sgp4 type drag coefficient              kg/m2er
*    ecco        - eccentricity
*    epoch       - epoch time in days from jan 0, 1950. 0 hr
...
elsetrec_t *SGP4Funcs_sgp4init (gravconst_t whichconst, char opsmode,
    ...
    double ao, ainv, con42, cosio, sinio, cosio2, eccsq, omeosq, posq, rp,
        ...
        coef, coef1, cosio4, day, dndt, em, emsq, eeta, etasq, gam, argpm,
        nodem, inclm, mm, nm, perige, pinvsq, psisq, qzms24, rtemsq, s1, s2, s3,
        s4, s5, s6, s7, sfour, ss1, ss2, ss3, ss4, ss5, ss6, ss7, sz1, sz2, sz3,
        sz11, sz12, sz13, sz21, sz22, sz23, sz31, sz32, sz33, tc, temp, temp1,
        temp2, temp3, tsi, xpidot, xhdot1, z1, z2, z3, z11, z12, z13, z21, z22,
        z23, z31, z32, z33, qzms2t, ss, x2o3, r[3], v[3], delmotemp, qzms2ttemp,
        qzms24temp;
    ...
    SGP4Funcs_getgravconst (whichconst, &satrec->tumin, &satrec->mu,
    ...
    // satrec->
    // it is possible to streamline the SGP4Funcs_sgp4init call by deleting the
    // "x"
    // variables, but the user would need to set the satrec->* values first. we
    // include the additional assignments in case SGP4Funcs_twoline2rv is not
    // used.
    satrec->bstar = xbstar;
    ...
    satrec->am = satrec->em = satrec->im = satrec->Om = satrec->mm =
        satrec->nm = 0.0;
    ...
    initl (satrec->xke, satrec->j2, satrec->ecco, epoch, satrec->inclo,
           ...
           &rteosq, &sinio, &satrec->gsto, &satrec->no_unkozai);
    ...
    // the mrt check in SGP4Funcs_sgp4 handles decaying satellite cases even if
    // the
    // starting
    ...
    if ((omeosq >= 0.0) || (satrec->no_unkozai >= 0.0)) {
        ...
        if ((2 * pi / satrec->no_unkozai) >= 225.0) {
            ...
            dscom (epoch, satrec->ecco, satrec->argpo, tc, satrec->inclo,
                   satrec->nodeo, satrec->no_unkozai, &snodm, &cnodm, &sinim,
                   &cosim, &sinomm, &cosomm, &day, &satrec->e3, &satrec->ee2,
                   &em, &emsq, &gam, &satrec->peo, &satrec->pgho, &satrec->pho,
                   &satrec->pinco, &satrec->plo, &rtemsq, &satrec->se2,
                   &satrec->se3, &satrec->sgh2, &satrec->sgh3, &satrec->sgh4,
                   &satrec->sh2, &satrec->sh3, &satrec->si2, &satrec->si3,
                   &satrec->sl2, &satrec->sl3, &satrec->sl4, &s1, &s2, &s3, &s4,
                   &s5, &s6, &s7, &ss1, &ss2, &ss3, &ss4, &ss5, &ss6, &ss7,
                   &sz1, &sz2, &sz3, &sz11, &sz12, &sz13, &sz21, &sz22, &sz23,
                   &sz31, &sz32, &sz33, &satrec->xgh2, &satrec->xgh3,
                   &satrec->xgh4, &satrec->xh2, &satrec->xh3, &satrec->xi2,
                   &satrec->xi3, &satrec->xl2, &satrec->xl3, &satrec->xl4, &nm,
                   &z1, &z2, &z3, &z11, &z12, &z13, &z21, &z22, &z23, &z31,
            ...
            dpper (satrec->e3, satrec->ee2, satrec->peo, satrec->pgho,
                   ...
                   satrec->xl3, satrec->xl4, satrec->zmol, satrec->zmos, inclm,
                   satrec->init, &satrec->ecco, &satrec->inclo, &satrec->nodeo,
                   &satrec->argpo, &satrec->mo, satrec->operationmode);
            ...
            dsinit (satrec->xke, cosim, emsq, satrec->argpo, s1, s2, s3, s4, s5,
                    ...
                    satrec->mdot, satrec->no_unkozai, satrec->nodeo,
                    satrec->nodedot, xpidot, z1, z3, z11, z13, z21, z23, z31,
                    z33, satrec->ecco, eccsq, &em, &argpm, &inclm, &mm, &nm,
                    &nodem, &satrec->irez, &satrec->atime, &satrec->d2201,
                    &satrec->d2211, &satrec->d3210, &satrec->d3222,
                    &satrec->d4410, &satrec->d4422, &satrec->d5220,
                    &satrec->d5232, &satrec->d5421, &satrec->d5433,
                    &satrec->dedt, &satrec->didt, &satrec->dmdt, &dndt,
                    &satrec->dnodt, &satrec->domdt, &satrec->del1,
                    &satrec->del2, &satrec->del3, &satrec->xfact,
...
/*-----------------------------------------------------------------------------
*
*                             procedure SGP4Funcs_sgp4
*
*  this procedure is the SGP4Funcs_sgp4 prediction model from space command.
this is an
*    updated and combined version of SGP4Funcs_sgp4 and sdp4, which were
originally
...
*
*  inputs        :
*    satrec	 - initialised structure from SGP4Funcs_sgp4init() call.
...
*
*  coupling      :
*    SGP4Funcs_getgravconst- no longer used. Variables are conatined within
satrec
*    dpper
*    dpspace
*
...
bool SGP4Funcs_sgp4 (elsetrec_t *satrec, double tsince, double r[3],
                     double v[3]) {
    double am, axnl, aynl, betal, cosim, cnod, cos2u, coseo1, cosi, cosip,
        ...
        mrt = 0.0, mvt, rdotl, rl, rvdot, rvdotl, sinim, sin2u, sineo1, sini,
        sinip, sinsu, sinu, snod, su, t2, t3, t4, tem5, temp, temp1, temp2,
        tempa, tempe, templ, u, ux, uy, uz, vx, vy, vz, inclm, mm, nm, nodem,
        xinc, xincp, xl, xlm, mp, xmdf, xmx, xmy, nodedf, xnode, nodep, tc,
        dndt, twopi, x2o3, vkmpersec, delmtemp;
    int ktr;
    ...
    /* --------------------- clear SGP4Funcs_sgp4 error flag -----------------
     */
    satrec->t = tsince;
    satrec->error = 0;
    ...
    if (satrec->method == 'd') {
        ...
        dspace (satrec->irez, satrec->d2201, satrec->d2211, satrec->d3210,
                ...
                satrec->del1, satrec->del2, satrec->del3, satrec->didt,
                satrec->dmdt, satrec->dnodt, satrec->domdt, satrec->argpo,
                satrec->argpdot, satrec->t, tc, satrec->gsto, satrec->xfact,
                satrec->xlamo, satrec->no_unkozai, &satrec->atime, &em, &argpm,
                &inclm, &satrec->xli, &mm, &satrec->xni, &nodem, &dndt, &nm);
    } // if method = d
    ...
    if (satrec->method == 'd') {
        dpper (satrec->e3, satrec->ee2, satrec->peo, satrec->pgho, satrec->pho,
               satrec->pinco, satrec->plo, satrec->se2, satrec->se3,
               satrec->sgh2, satrec->sgh3, satrec->sgh4, satrec->sh2,
               satrec->sh3, satrec->si2, satrec->si3, satrec->sl2, satrec->sl3,
               satrec->sl4, satrec->t, satrec->xgh2, satrec->xgh3, satrec->xgh4,
               satrec->xh2, satrec->xh3, satrec->xi2, satrec->xi3, satrec->xl2,
...
/* -----------------------------------------------------------------------------
*
*                           function SGP4Funcs_getgravconst
*
...
void SGP4Funcs_getgravconst (gravconst_t whichconst, double *tumin, double *mu,
    ...
    case wgs72:
        ...
        *radiusearthkm = 6378.135; // km
        *xke = 60.0 /
               sqrt (*radiusearthkm * *radiusearthkm * *radiusearthkm / *mu);
        *tumin = 1.0 / *xke;
        *j2 = 0.001082616;
        *j3 = -0.00000253881;
        *j4 = -0.00000165597;
...
/* -----------------------------------------------------------------------------
        ...
        *
        *  author        : david vallado                  719-573-2600    1 mar
 * 2001
        *

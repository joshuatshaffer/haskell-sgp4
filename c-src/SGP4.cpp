...
static void initl (
    ...
    // gravconsttype whichconst,
    double xke, double j2, double ecco, double epoch, double inclo, double &no,
    char &method, double &ainv, double &ao, double &con41, double &con42,
...
bool sgp4init (gravconsttype whichconst, char opsmode, const int satn,
    ...
    initl (satrec.xke, satrec.j2, satrec.ecco, epoch, satrec.inclo,
           ...
           sinio, satrec.gsto, satrec.no_unkozai);
    ...
    if ((omeosq >= 0.0) || (satrec.no_unkozai >= 0.0)) {
        ...
        if ((2 * pi / satrec.no_unkozai) >= 225.0) {
            ...
            dscom (epoch, satrec.ecco, satrec.argpo, tc, satrec.inclo,
                   ...
                   satrec.si3, satrec.sl2, satrec.sl3, satrec.sl4, s1, s2, s3,
                   s4, s5, s6, s7, ss1, ss2, ss3, ss4, ss5, ss6, ss7, sz1, sz2,
                   sz3, sz11, sz12, sz13, sz21, sz22, sz23, sz31, sz32, sz33,
                   satrec.xgh2, satrec.xgh3, satrec.xgh4, satrec.xh2,
                   satrec.xh3, satrec.xi2, satrec.xi3, satrec.xl2, satrec.xl3,
                   satrec.xl4, nm, z1, z2, z3, z11, z12, z13, z21, z22, z23,
            ...
            dsinit (satrec.xke, cosim, emsq, satrec.argpo, s1, s2, s3, s4, s5,
                    ...
                    z33, satrec.ecco, eccsq, em, argpm, inclm, mm, nm, nodem,
                    satrec.irez, satrec.atime, satrec.d2201, satrec.d2211,
                    satrec.d3210, satrec.d3222, satrec.d4410, satrec.d4422,
                    satrec.d5220, satrec.d5232, satrec.d5421, satrec.d5433,
                    satrec.dedt, satrec.didt, satrec.dmdt, dndt, satrec.dnodt,
                    satrec.domdt, satrec.del1, satrec.del2, satrec.del3,
...
bool sgp4 (elsetrec &satrec, double tsince, double r[3], double v[3]) {
    ...
    if (satrec.method == 'd') {
        dpper (satrec.e3, satrec.ee2, satrec.peo, satrec.pgho, satrec.pho,
               ...
               satrec.zmol, satrec.zmos, satrec.inclo, 'n', ep, xincp, nodep,
...
varaibles
*    and quantities are determined. note that the result is a structure so
multiple
...
*    verification mode. the catalog mode simply propagates from -1440 to 1440
min
...
*                                                 manual 'm'
*    typeinput   - type of manual input           mfe 'm', epoch 'e', dayofyr
'd'
*    opsmode     - mode of operation afspc or improved 'a', 'i'
*    whichconst  - which set of constants to use  72, 84
*
...
void twoline2rv (char longstr1[130], char longstr2[130], char typerun,
    ...
    sscanf_s (longstr1,
              ...
              &cardnumb, &satrec.satnum, &satrec.classification, sizeof (char),
              &satrec.intldesg, 11 * sizeof (char), &satrec.epochyr,
              &satrec.epochdays, &satrec.ndot, &satrec.nddot, &nexp,
              &satrec.bstar, &ibexp, &satrec.ephtype, &satrec.elnum);
#else
    sscanf (longstr1,
            "%2d %5ld %1c %10s %2d %12lf %11lf %7lf %2d %7lf %2d %2d %6ld ",
            &cardnumb, &satrec.satnum, &satrec.classification, &satrec.intldesg,
    ...
    if (typerun == 'v') // run for specified times from the file
    {
        if (longstr2[52] == ' ') {
            ...
            sscanf_s (
                ...
                &satrec.ecco, &satrec.argpo, &satrec.mo, &satrec.no_kozai,
...
// make sure there is no space at the end of the format specifiers in scanf!
#ifdef _MSC_VER
            scanf_s ("%i %i %i %i %i %lf", &startyear, &startmon, &startday,
...
#ifdef _MSC_VER
            scanf_s ("%i %i %i %i %i %lf", &stopyear, &stopmon, &stopday,
            ...
            startmfe = (jdstart - satrec.jdsatepoch) * 1440.0 +
                       (jdstartF - satrec.jdsatepochF) * 1440.0;
            stopmfe = (jdstop - satrec.jdsatepoch) * 1440.0 +
                      (jdstopF - satrec.jdsatepochF) * 1440.0;
...
void rv2coe (double r[3], double v[3], const double mu, double &p, double &a,
    ...
    int i;
    // switch this to an integer msvs seems to have probelms with this and
    // strncpy_s
    // char typeorbit[2];
    int typeorbit;
    // here
